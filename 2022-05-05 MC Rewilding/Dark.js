...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/02 modularity color.gexf', 'utf8');
...
settings.image_width = 594 // in mm. Default: 200mm (fits in a A4 page)
settings.image_height = 420
...
settings.margin_top    = 32 // in mm
settings.margin_right  = 32 // in mm
settings.margin_bottom = 32 // in mm
settings.margin_left   = 128 // in mm
...
// Layer: Background
settings.background_color = "#4d5b38"
...
// Layer: Edges
settings.edge_thickness = .08 // in mm
...
settings.edge_color = "#70645A"

// Layer: Node shadows
settings.node_color_shadow_offset = 32 // mm; larger than you'd think (gradient)
settings.node_color_shadow_opacity = .2
settings.node_color_shadow_blur_radius = 4 // mm

// Layer: Nodes
settings.adjust_voronoi_range = 100 // Factor // Larger node halo
settings.node_size = .5 // Factor to adjust the nodes drawing size
...
settings.label_border_color = "#000000"
...
// Advanced settings
settings.voronoi_range = 1.5 // Halo size in mm
...
settings.heatmap_spreading = settings.image_width / 32 // in mm

// Experimental stuff
settings.hillshading_strength = 1
settings.hillshading_color = "#00739b"
settings.hillshading_alpha = .5 // Opacity
settings.hillshading_sun_azimuth = Math.PI * 0.7 // angle in radians
settings.hillshading_sun_elevation = Math.PI * 0.22 // angle in radians
settings.hillshading_hypsometric_gradient = true // Elevation gradient color
settings.hillshading_hypsometric_gradient_domain = [0,         0.30,      0.45,      1]
settings.hillshading_hypsometric_gradient_colors = ['#332211', '#443333', '#665555', '#776677']
...
g.nodes().forEach(nid => {
  ...
  n.showLabel = true//n.attr_type == "Author Keywords"
})
...
newRenderer = function(){
  ...
  ns.render = function(g, settings) {
    ...
    var layeredImage = ns.getEmptyLayer(false)
  ...
  ns.drawHillshadingGradient = function(options) {
    ...
    options.hillshading_hypsometric_gradient_domain = options.hillshading_hypsometric_gradient_domain || [0, 0.2*0.8, 0.2*1.2, 1]
    options.hillshading_hypsometric_gradient_colors = options.hillshading_hypsometric_gradient_colors || ['#666666', '#BBBBBB', '#EEEEEE', '#F6F6F6']
    ...
    if (options.display_heatmap) {
      ...
      if (options.hillshading_hypsometric_gradient) {
        ...
        let colorGradient = d3.scaleLinear()
          .domain(settings.hillshading_hypsometric_gradient_domain)
          .range(settings.hillshading_hypsometric_gradient_colors)
  ...
  ns.tuneColorForLabel = function(c) {
    ...
    options.label_color_min_L = 35 // Custom
    options.label_color_max_L = 65 // Custom
    var hcl = d3.hcl(c)
  ...
  ns.drawNodesShadowLayer = function(options) {
    ...
    if (ns._nodeShadows) {
      ...
      ns.paintAll(ctx, 'rgba(0, 0, 0, '+(1-options.node_color_shadow_opacity)+')') // Custom
      ns.report("...done.")
      return ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height)
      ...
      while (--steps>0) {
        ...
        ns.getNodesBySize().forEach(function(nid){
          ...
          // CUSTOM
          // Tune the color to be a bit more vivid, a bit MORE dark
          var hsl = d3.hsl(color)
          hsl.l = Math.min(1, hsl.l * 0.8)
          hsl.s = Math.min(1, hsl.s * 1.1)

          // CUSTOM
          // Bluriness (actually DARKness)
          hsl.l = (1-layerOpacity)*0 + layerOpacity*hsl.l
