...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/10 couleur politique V3.gexf', 'utf8');
...
// Layer: Node shadows
settings.node_color_shadow_offset = 10 // mm; larger than you'd think (gradient)
...
settings.node_size = 2. // Factor to adjust the nodes drawing size
...
settings.label_font_min_size = 5.5 // in pt
settings.label_font_max_size = 7.5  // in pt
settings.label_font_thickness = .22
...
let badSeed = 707 // LOL
function bsRandom() {
    var x = Math.sin(badSeed++) * 10000;
    return x - Math.floor(x);
}
let lols = []
let lolsindex = {}
g.nodes()
.filter(nid => ['Ext Gauche', 'ext Gauche', 'Gauche', 'Ecolo', 'Centre', 'Droite', 'Souverainiste', 'MLP', 'Reconquête', 'Antisystème', 'Media', 'media'].indexOf(g.getNodeAttribute(nid, 'couleur politique'))>=0)
.forEach(nid => {
  let lol = bsRandom()
  lols.push(lol)
  lolsindex[lol] = g.getNodeAttribute(nid, 'couleur politique')
})
lols.sort()
lols.forEach((lol,i) => {
  mod = lolsindex[lol]
  g.setNodeAttribute(g.nodes()[i], 'couleur politique', mod)
})
...
g.nodes().forEach(nid => {
  ...
  if (n.important) {
    ...
    // Also tune the starting angle of the label (trigo convention)
    if (nid == '1003575248'){ // Nathalie Arthaud
      n.labelStartAngle = (4/5)*Math.PI
    } else if (nid == '38170599'){ // Nicolas Dupont-Aignan
      n.labelStartAngle = -Math.PI/6
    } else if (nid == '26073581'){ // Anne Hidalgo
      n.labelStartAngle = Math.PI/3
    } else if (nid == '117761523'){ // Yannick Jadot
      // n.labelStartAngle = 0
    } else if (nid == '102722347'){ // Jean Lassalle
      n.labelStartAngle = Math.PI/2
    } else if (nid == '217749896'){ // Marine Le Pen
      // n.labelStartAngle = 0
    } else if (nid == '1976143068'){ // Emmanuel Macron
      n.labelStartAngle = Math.PI/3
    } else if (nid == '80820758'){ // Jean-Luc Mélenchon
      n.labelStartAngle = -Math.PI/3
    } else if (nid == '49969223'){ // Valérie Pécresse
      // n.labelStartAngle = 0
    } else if (nid == '374392774'){ // Philippe Poutou
      n.labelStartAngle = -Math.PI/4
    } else if (nid == '1324449588'){ // Fabien Roussel
      // n.labelStartAngle = 0
    } else if (nid == '1183418538285027329'){ // Eric Zemmour
      // n.labelStartAngle = 0
    } 
  }
})
...
newRenderer = function(){
  ...
  ns.getVisibleLabels = function(options) {
    ...
    var nodesBySize = ns.getNodesBySize()
      .filter(nid => !g.getNodeAttribute(nid, "hidden")) // Custom
      .slice(0)
    nodesBySize.reverse()
    ...
    if (ns._nodeShadows) {
      ...
      while (--steps>0) {
        ...
        ns.getNodesBySize()
          ...
          .filter(nid => !g.getNodeAttribute(nid, 'hidden')) // Custom filter
          .forEach(function(nid){
            var n = g.getNodeAttributes(nid)
...
// TROGNEUX INDEX // Custom
let trogneuxIndex = {}
fs.createReadStream('data/users_Trogneux_polemic.csv')
  .pipe(csv())
  .on('data', (row) => {
    trogneuxIndex[row.twitterID] = true
  })
  .on('end', () => {
    ...
    /*g.nodes().forEach(nid => {
      let n = g.getNodeAttributes(nid)
      n.hidden = !trogneuxIndex[nid]
    })*/

    renderer.renderAndSave(g, settings, 'Standard swaped') // Custom
  });
