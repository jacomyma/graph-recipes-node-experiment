...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/Cultural Events DK -mj.gexf', 'utf8');
...
settings.margin_top    = 196 // in mm
settings.margin_right  =  48 // in mm
settings.margin_bottom =  24 // in mm
settings.margin_left   =  32 // in mm
...
settings.cc_text_border_color = "#e3d6c1"
settings.cc_font_size = 12 // in pt
settings.cc_line_color = "#764301"
settings.cc_grid_line_color = "#daa96d"
settings.cc_info_margin_offset = 9 // In mm
...
settings.cluster_node_size_margin = 1 // In mm
settings.cluster_shape_smoothness = 40 // In mm (underlying blur)
settings.cluster_shape_size = 1 // Range: more than 0, default to 1.
settings.cluster_shape_swelling = 0.8 // Range: 0.01 to 0.99 // Balanced: 0.5 // Acts on size
// ...cluster fills
...
settings.cluster_label_font_max_size = 32 // In pt
settings.cluster_label_font_thickness = .45 // In mm
settings.cluster_label_border_thickness = 1.8 // In mm
...
// Advanced settings
settings.voronoi_range = 5 // Halo size in mm
settings.voronoi_resolution_max = 1 * Math.pow(10, 7) // in pixel. 10^7 still quick, 10^8 better quality 
settings.heatmap_resolution_max = 5 * Math.pow(10, 5) // in pixel. 10^5 quick. 10^7 nice but super slow.
...
newRenderer = function(){
  ...
  // Render all tiles
  ns.renderAndSaveAllTiles = function(g, settings, xstart, ystart) {
    xstart = xstart || 0
    ystart = ystart || 0

    console.log("Rendering all tiles.")
    if (xstart>0 || ystart>0) {
      console.log("(starting from "+xstart+","+ystart+")")
    }
    ...
    settings.tile_to_render = [xstart, ystart]
    ns.renderAndSaveTile(g, settings)

  }

  ns.renderAndSaveTile = function(g, settings) {
    console.log("\n###### DRAW TILE "+settings.tile_to_render[0]+" "+settings.tile_to_render[1]+" ("+(1+settings.tile_to_render[0]+settings.tile_to_render[1]*settings.tile_factor)+"/"+Math.pow(settings.tile_factor,2)+") ######")
    let tile = ns.render(g, settings)
    renderer.saveCanvas(tile, "tile "+settings.tile_to_render[0]+" "+settings.tile_to_render[1], () => {
      console.log("Tile "+settings.tile_to_render[0]+" "+settings.tile_to_render[1]+".png saved.")
      settings.tile_to_render[0]++
      if (settings.tile_to_render[0]>=settings.tile_factor) {
        settings.tile_to_render[0] = 0
        settings.tile_to_render[1]++
      }
      if (settings.tile_to_render[1]<settings.tile_factor){
        ns.renderAndSaveTile(g, settings)
      } else {
  ...
  ns.drawNodeLabelsLayer = function(options) {
    ...
    visibleLabels.forEach(function(nid){
      ...
      */
      ...
      var labelCoordinates = {
        x: nx + 0.6 * borderThickness + 1.05 * radius,
        y: ny + 0.25 * fontSize
      }
  ...
  ns.getBBox = function(ctx, fontSize, labelCoordinates, label, factor, offset) {
    ...
    // We remove an additional square on the left
    // to incorporate the node itself.
    return {
      x: x - ymargin - h,
      y: y - ymargin,
      width: w + 2*ymargin + h,
  ...
  ns.drawEdgesLayer = function(options) {
    ...
    options.edge_path_segment_length = options.edge_high_quality?.133:2 // in mm
    // Monitoring options
  ...
  ns.drawNodesLayer = function(options) {
    ...
    options.spacingFactor = options.spacingFactor || 6
    ...
    ns.getNodesBySize().forEach(function(nid){
      ...
      // Draw strokes
      for (var ref=options.spacingFactor*stroke_width-radius; ref < radius; ref += options.spacingFactor*stroke_width) {
        ...
        ctx.beginPath()
        ctx.strokeStyle = options.node_stroke_color
        ctx.moveTo(x0, y0)
        ctx.lineTo(x1, y1)
        ctx.stroke()
        ctx.closePath()
