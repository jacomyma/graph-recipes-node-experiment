...
const filename = "0250 Black dots + lines - All borders"
...
settings.node_stroke_width = 1.5
...
newRenderer = function(){
  ...
  ns.drawEdgesLayer = function(options) {
    ...
    if (options.display_edges) {
      ...
      ctx.lineJoin="round"
      ctx.fillStyle = 'rgba(0, 0, 0, 0)';
      let edgesSorted = g.edges()
        .filter(function(eid, i_){ return i_ < options.max_edge_count })
      edgesSorted.forEach(eid => {
        let e = g.getEdgeAttributes(eid)
        let n_s = g.getNodeAttributes(g.source(eid))
        let n_t = g.getNodeAttributes(g.target(eid))
        e.sqlen = Math.pow(n_t.x-n_s.x, 2) + Math.pow(n_t.y-n_s.y, 2)
      })
      edgesSorted.sort(function(a,b){
        return g.getEdgeAttribute(b, "sqlen")-g.getEdgeAttribute(a, "sqlen")
      })
      ...
      edgesSorted
        .forEach(function(eid, i_){
          ...
          // Draw path in white (with border)
          for (i=0; i<path.length; i+=3) {
            ...
            if (lastx) {
              ctx.lineWidth = 3.5 * thickness * (0.9 + 0.2*Math.random())
