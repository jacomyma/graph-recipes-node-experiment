...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/06 with clusters.gexf', 'utf8');
...
settings.node_size = 1.1 // Factor to adjust the nodes drawing size
settings.node_color_original = false // Use the original node color
settings.node_stroke_color = settings.edge_color
...
settings.node_clusters = {
  ...
  "modalities": {
    ...
    "Extrême-droite (RN/Zemmour)": {
      "label": "Extrême-droite (RN/Zemmour)",
      "count": 505,
      "color": "#ce6028"
    },
    "Gauche/Opposition droites": {
      "label": "Gauche/Opposition droites",
      "count": 457,
      "color": "#f2a5a6"
    },
    "Zemmour1": {
    ...
    "Gauches (et LREM gauche)": {
      "label": "Gauches (et LREM gauche)",
      "count": 109,
      "color": "#b65887"
    },
    "Médias/Personnalités politiques droite": {
...
newRenderer = function(){
  ...
  ns.drawNodeLabelsLayer = function(options) {
    ...
    // Draw borders
    if (options.label_border_thickness > 0) {
      ctx.textAlign = "center"
      ctx.lineCap = "round"
      ctx.lineJoin = "round"
      labelsStack.forEach(function(l){
