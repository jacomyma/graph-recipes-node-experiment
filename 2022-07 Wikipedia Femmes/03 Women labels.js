...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/04 cleaned and spat.gexf', 'utf8');
...
settings.image_width = 700 // in mm. Default: 200mm (fits in a A4 page)
settings.image_height = 700
...
settings.rotate = 95 // In degrees, clockwise
...
settings.node_size = 3.5 // Factor to adjust the nodes drawing size
...
settings.label_font_max_size = 22  // in pt
settings.label_font_thickness = .45
settings.label_border_thickness = 1 // in mm
...
settings.label_spacing_offset = 0 // Spacing offset
settings.label_spacing_factor = 0.3 // Spacing offset
settings.label_path_starting_angle_range = Math.PI // From 0 (horizontal) to PI (any angle)
...
settings.heatmap_spreading = 7 // in mm

// Experimental stuff
settings.hillshading_strength = 25
settings.hillshading_color = "#979272"
settings.hillshading_alpha = .7 // Opacity
...
newRenderer = function(){
  ...
  // FIXED
  ns.getTextPathDraw = function(useBorder) {
    const letterAlignmentFactor = 0.3; // Proportional to the font size
    const letterAlignmentOffset = ns.mm_to_px(0); // Always the same
    if (useBorder) {
      return function(ctx, letter, x, y, angle) {
        let fontSize = +(ctx.font.match(/(bold )?([0-9\.]+) ?px.*/i)[2])
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(angle);
        ctx.translate(0, letterAlignmentFactor * fontSize);
  ...
  ns.getLabelPaths = function(options) {
    ...
    // var textPath_draw = ns.getTextPathDraw()
  ...
  ns.drawTextPath = function(ctx, path, label, useBorder) {
    ...
    let textPath_draw = ns.getTextPathDraw(useBorder)
    try {
      ns.textPath(ctx, label, path, textPath_measureText, textPath_draw, 'center'); // FIXED
    } catch(e) {
  ...
  ns.getVisibleLabels = function(options) {
    ...
    options = options || {}
    options.labelCollisionsAllowed = false
    ...
    if (options.labelCollisionsAllowed) return ns.g.nodes()
    ...
    var count = 0
    nodesBySize
    .filter(nid => g.getNodeAttribute(nid, "type") != "HOMME")
    .forEach(function(nid){
      if (labelDrawCount > 0) {
        var n = g.getNodeAttributes(nid)
        var nx = n.x
  ...
  ns.textPath = function(ctx, text, path, measure, draw, textAlign) { // FIXED
    var pathLength = 0;
    var p1 = path[0]
  ...
  ns.drawNodesLayer = function(options) {
    ...
    ns.getNodesBySize()
    .filter(nid => g.getNodeAttribute(nid, "type") != "HOMME") // Custom
    .forEach(function(nid){
      var n = g.getNodeAttributes(nid)
...
renderer.renderAndSave(g, settings, "03 Women labels")
