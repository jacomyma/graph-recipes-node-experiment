...
newRenderer = function(){
  ...
  ns.drawEdgesLayer = function(options) {
    ...
    options.edge_bundled = (options.edge_bundled===undefined)?(true):(options.edge_bundled) // CUSTOM
    ...
    if (options.display_edges) {
      ...
      if (options.edge_bundled) {
        ...
        const edges = g.edges()
          ...
          .filter(eid => g.getNodeAttribute(g.source(eid), "type") != "HOMME" && g.getNodeAttribute(g.target(eid), "type") != "HOMME") // Custom
          
        edges.forEach(function(eid, i_){
          if ((i_+1)%10000 == 0) {
            console.log("..."+(i_+1)/1000+"K edge paths prepared...")
          }
...
renderer.renderAndSave(g, settings, "04 Edges")
