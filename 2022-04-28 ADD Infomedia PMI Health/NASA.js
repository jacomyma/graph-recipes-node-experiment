...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/02 spat (size relative degree).gexf', 'utf8');
...
settings.image_width = 841 // in mm. Default: 200mm (fits in a A4 page)
settings.image_height = 594
...
settings.margin_right  = 128 // in mm
...
settings.draw_polygons              = true // Custom!
...
// Layer: Edges
settings.edge_thickness = .24 // in mm
...
// Advanced settings
settings.voronoi_range = 1. // Halo size in mm
...
// Experimental stuff
settings.hillshading_strength = 24
...
let wMax = 0
g.edges().forEach(eid => {
  let w = g.getEdgeAttribute(eid, 'pmi')
...
g.nodes().forEach(nid => {
  ...
  n.showLabel = true//n.type == "ORG" || n.type == "PER"
})
...
newRenderer = function(){
  ...
  ns.render = function(g, settings) {
    ...
    if (ns.settings.draw_polygons && basemapPolygons && basemapPolygons.length > 0) {
      bgImage = ns.drawLayerOnTop(bgImage,
