...
// Layer: Node labels
settings.label_color = "#000000"//"#283535"
settings.label_color_from_node = false
settings.label_count = 2000
...
settings.label_font_max_size = 6  // in pt
...
// Fix underscores in labels
g.nodes().forEach(nid => {
  let n = g.getNodeAttributes(nid)
  n.label = n.label.replace(/_/gi, " ")
})
...
newRenderer = function(){
  ...
  ns.drawHillshadingGradient = function(options) {
    ...
    if (options.display_heatmap) {
      ...
      let imgd = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height) // Custom
...
renderer.renderAndSave(g, settings, "hipsometrics")
