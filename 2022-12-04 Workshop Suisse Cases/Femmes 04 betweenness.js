...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/femmes-wikipedia.gexf', 'utf8');
...
settings.image_width = 290 // in mm. Default: 200mm (fits in a A4 page)
...
settings.node_color_shadow_opacity = 0.96
...
// Resize
const sizeAttribute = "betweenesscentrality"
// const sizeAttribute = "closnesscentrality"
let saExtent = d3.extent(g.nodes().map(nid => g.getNodeAttribute(nid, sizeAttribute)))
let saScale = d3.scalePow()
  .exponent(1) // Curve shape. 1=linear.
  .domain(saExtent)
  .range([2, 20])
g.nodes().forEach(nid => {
  g.setNodeAttribute(nid, "size", saScale(g.getNodeAttribute(nid, sizeAttribute)) )
})
...
renderer.renderAndSave(g, settings, 'Femmes 04 betweenness') // Custom
