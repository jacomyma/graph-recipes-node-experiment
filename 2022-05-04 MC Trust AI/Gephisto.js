...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/01 Giant component + spat.gexf', 'utf8');
...
// Layer: Background
settings.background_color = "#B1B9C4"
...
settings.node_stroke_color = "#8A87AA00"
settings.node_fill_color = "#171637"

// Layer: Node labels
settings.label_color = "#7F2F39"
...
g.nodes().forEach(nid => {
  ...
  n.showLabel = n.attr_type!="Authors"
  n.color = (n.attr_type!="Authors")?("#C95B63"):("#616D92")
  if (nid=="authorkeywords_0fdfc5af25d6000471a2f39c268e823c") {
    n.color = "#FFE0D9"
  } else if (nid=="authorkeywords_e04d1bcee667afb8622501b9a4b4654d") {
    n.color = "#C49500"
  }
  n.label = n.label.toUpperCase()
})
g.edges().forEach(eid => {
  const e = g.getEdgeAttributes(eid)
  e.color = "#8A87AA"
  e.thickness = 1;
  nsid = g.source(eid)
  ntid = g.target(eid)
  if (nsid=="authorkeywords_0fdfc5af25d6000471a2f39c268e823c" || ntid=="authorkeywords_0fdfc5af25d6000471a2f39c268e823c") {
    e.thickness = 12
    e.color = "#FFBDAB"
  }
  if (nsid=="authorkeywords_e04d1bcee667afb8622501b9a4b4654d" || ntid=="authorkeywords_e04d1bcee667afb8622501b9a4b4654d") {
    e.thickness = 1
    e.color = "#BC9002"
  }
})
...
newRenderer = function(){
  ...
  ns.drawNodeLabelsLayer = function(options) {
    ...
    var visibleLabels = ns.getVisibleLabels(options, normalizeFontSize)
      .filter(nid => g.getNodeAttribute(nid, 'showLabel'))
    ...
    if (options.display_edges) {
      ...
      g.edges()
        ...
        .forEach(function(eid, i_){
          ...
          var color = d3.color(g.getEdgeAttribute(eid, "color")) // Custom
          var thickness = ns.mm_to_px(options.edge_thickness * g.getEdgeAttribute(eid, "thickness")) // Thickness
