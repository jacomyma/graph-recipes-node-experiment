...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/05 nodes colored.gexf', 'utf8');
...
settings.image_width = 1.5*200 // in mm. Default: 200mm (fits in a A4 page)
settings.image_height = 1.5*200
...
settings.edge_alpha = .95 // Opacity // Range from 0 to 1
...
settings.node_size = 1.4 // Factor to adjust the nodes drawing size
...
settings.node_clusters = {
  ...
  "modalities": {
    "heterogene/not_known": {
      "label": "heterogene/not_known",
      "count": 6418,
      "color": "#6fc5a4"
    },
    "Anti??Troll?": {
    ...
    "Antisystème/ AntiMacron": {
      "label": "Antisystème/ AntiMacron",
      "count": 2975,
      "color": "#b9a2ce"
    },
    "Zemmour 1": {
    ...
    "Doite Anti Zemmour 2": {
      "label": "Doite Anti Zemmour 2",
      "count": 786,
      "color": "#f2a5a6"
    },
    "Mélenchon 1": {
    ...
    "Doite Anti Zemmour 1": {
      "label": "Doite Anti Zemmour 1",
      "count": 273,
      "color": "#b65887"
    },
    "Zemmour 5": {
    ...
    "Soutien Mélenchon/ Média": {
      "label": "Soutien Mélenchon/ Média",
      "count": 143,
      "color": "#9d9b99"
    },
    "Doite Anti Zemmour / personalités politiques / média": {
      "label": "Doite Anti Zemmour / personalités politiques / média",
      "count": 87,
      "color": "#9d9b99"
    },
    "Zemmour 3": {
    ...
    "Personalités politiques gauche/ Extrême gauche": {
      "label": "Personalités politiques gauche/ Extrême gauche",
      "count": 37,
      "color": "#9d9b99"
    },
    "Entourage Zemmour/ Medias + MLP": {
      "label": "Entourage Zemmour/ Medias + MLP",
      "count": 13,
      "color": "#9d9b99"
    },
    "Personalités Droite/Antisystème": {
      "label": "Personalités Droite/Antisystème",
      "count": 9,
      "color": "#9d9b99"
    },
    "Personalités politiques PecresseMacron": {
      "label": "Personalités politiques PecresseMacron",
      "count": 8,
      "color": "#9d9b99"
    },
    "3 candidats": {
    ...
    "Candidats gauche/ média": {
      "label": "Candidats gauche/ média",
      "count": 3,
      "color": "#9d9b99"
    }
  },
...
const modality = 'Candidats gauche/ média'
g.nodes().forEach(nid => {
  var n = g.getNodeAttributes(nid)
  n.hidden = n.cluster != modality
})

/*const edgeTopic = 'topic8'
g.edges().forEach(eid => {
...
newRenderer = function(){
  ...
  ns.drawEdgesLayer = function(options) {
    ...
    // Draw each edge
    // var color = d3.color(options.edge_color) // See below: edges colored as their target
    ...
    if (options.display_edges) {
      ...
      g.edges()
        ...
        .forEach(function(eid, i_){
          ...
          var color = d3.color(g.getNodeAttribute(g.target(eid), 'color'))
          if ((i_+1)%10000 == 0) {
  ...
  ns.drawNodesLayer = function(options) {
    ...
    options.node_size_offset = .33 //mm
    ...
    ns.getNodesBySize().forEach(function(nid){
      ...
      if (!n.hidden) {
        ...
        var radius = Math.max(options.node_size * n.size, stroke_width) + ns.mm_to_px(options.node_size_offset)
...
const filename = "bloc "+modality.replace(regex, '').trim()
