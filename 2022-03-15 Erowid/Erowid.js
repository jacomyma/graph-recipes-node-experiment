...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/title-word2vec SPAT.gexf', 'utf8');
...
settings.image_width = 800 // in mm. Default: 200mm (fits in a A4 page)
settings.image_height = 1100
...
// Layer: Edges
settings.edge_thickness = 0.15 // in mm
...
settings.label_max_length = 54 // Number of characters before truncate. Infinity is a valid value.
settings.label_font_family = "Raleway"
settings.label_font_min_size = 7 // in pt
settings.label_font_max_size = 18  // in pt
settings.label_font_thickness = .4
...
settings.heatmap_spreading = settings.image_width / 80 // in mm

// Experimental stuff
settings.hillshading_strength = 48
  ...
  ns.drawHillshadingGradient = function(options) {
    ...
    if (options.display_heatmap) {
      ...
      if (options.hillshading_hypsometric_gradient) {
        let mid_threshold = 0.15
        ns.log2("Draw hypsometric gradient...")
        let colorGradient = d3.scaleLinear()
          .domain([0, mid_threshold*0.666, mid_threshold*1.333, 1])
          .range(['#271028', '#B6ABAB', '#F5F1EF', '#FFFFFF'])
