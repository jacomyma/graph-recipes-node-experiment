...
var gexf_string;
try {
    gexf_string = fs.readFileSync('data/07 with political color.gexf', 'utf8');
...
settings.edge_alpha = .3 // Opacity // Range from 0 to 1
...
newRenderer = function(){
  ...
  ns.getVisibleLabels = function(options) {
    ...
    var count = 0
    nodesBySize
    .filter(nid => g.getNodeAttribute(nid, 'showlabel')==1) // Custom
    .forEach(function(nid){
      if (labelDrawCount > 0) {
        var n = g.getNodeAttributes(nid)
        var nx = n.x
