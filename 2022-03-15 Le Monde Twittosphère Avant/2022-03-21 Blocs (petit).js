...
settings.node_size = 1.8 // Factor to adjust the nodes drawing size
...
// Custom
let blocs = [
  "Autorités antisystèmes",
  "Autorités de la droite et du centre",
  "Divers comptes des partis traditionnels",
  "Acteurs majeurs 1 : Macron, Pécresse, Zemmour",
  "Autorités de la droite et de l’extrême droite",
  "Comptes de la droite et l’extrême droite",
  "Comptes peu influents de droite, de l’extrême droite, et antisystèmes",
  "Acteurs majeurs 2 : Jadot, Mélenchon, BFMTV",
  "Autorités de l’extrême gauche",
  "Comptes influents à l’extrême gauche",
  "Comptes peu influents de la gauche et de l’extrême gauche",
  "Autres",
]
blocs.forEach(bloc => {
  g.nodes().forEach(nid => {
    let n = g.getNodeAttributes(nid)
    n.hidden = n.cluster2 != bloc
  })
  console.log("### BLOC "+bloc, g.nodes().filter(nid => {
    let n = g.getNodeAttributes(nid)
    return n.hidden
  }).length + ' nodes hidden')
  let renderer = newRenderer()
  renderer.renderAndSave(g, settings, "bloc "+bloc.replace(/[^a-zA-Z0-9]/gi, ''))
})
